...
def query_and_write_to_csv(connection, partition_name, csv_writer):
    query = sql.SQL("""
    SELECT utc_date, placekey, location_name, naics_code, street_address, city, state, census_block_group, local_timestamp, caid, id_type, top_category, sub_category, brands, zip_code, minimum_dwell, geohash_5
    FROM {}
            ...
            for m in [8]:
                ...
                if m<12:
                    ...
                    while start_date <= end_date:
                        CSV_FILE = "/shared/veraset/shared/XIBO/transportation/raw_data/trans_"+start_date+".csv"
                    ...
                    print(end_date) 
                    while start_date <= end_date:
                        CSV_FILE = "/shared/veraset/shared/XIBO/food_env/raw_data/foodenv_"+start_date+".csv"
                        with open(CSV_FILE, mode='w', newline='') as csv_file:
                            csv_writer = csv.writer(csv_file)
                            csv_writer.writerow(['utc_date', 'placekey', 'location_name', 'naics_code', 'street_address', 'city', 'state',
                                  'census_block_group', 'local_timestamp', 'caid', 'id_type', 'top_category', 'sub_category', 'brands',
